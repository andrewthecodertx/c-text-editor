...
EditorConfig *get_editor_config() { return &E; }
...
EditorSelectionRange editor_resolve_selection() {
  ...
  if (E.sel_start_row < E.cy ||
      (E.sel_start_row == E.cy && E.sel_start_col <= E.cx)) {
  ...
  case KEY_MOUSE: {
    ...
    if (getmouse(&event) == OK) {
      if (event.bstate & BUTTON1_CLICKED) {
        ...
        EditorLine *line =
            (E.cy < E.lines.size) ? &E.lines.elements[E.cy] : NULL;
        int line_len = line ? line->len : 0;
        if (E.cx > line_len) {
          E.cx = line_len;
        }
...
void editor_insert_char(int c) {
  EditorAction action = {.type = ACTION_INSERT_CHAR,
                         .row = E.cy,
                         .col = E.cx,
                         .character = (char)c};
...
int editor_insert_newline() {
  EditorAction action = {
      .type = ACTION_INSERT_NEWLINE, .row = E.cy, .col = E.cx};
  editor_record_action(action);
  if (E.lines.size == 0) {
    EditorLine new_line = {
        .text = strdup(""), .len = 0, .hl = NULL, .hl_open_comment = 0};
    if (new_line.text == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Out of memory for initial line text.");
      return -1;
    }
    editor_lines_array_append(&E.lines, new_line);
    E.cy = 0;
  ...
  EditorLine new_line = {
      .text = NULL, .len = 0, .hl = NULL, .hl_open_comment = 0};
  ...
  if (E.cx == 0) {
    ...
    if (E.lines.elements[E.cy].text == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Out of memory for new empty line text.");
      return -1;
    }
    ...
    if (E.lines.elements[E.cy + 1].text == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Out of memory for split line text.");
      return -1;
    }
    ...
    if (current_line->text == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Out of memory for truncated line.");
      return -1;
    }
...
void editor_del_char() {
  ...
  if (E.cx > 0) {
    ...
    if (line->text == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Out of memory (del char realloc).");
      return;
    }
    E.cx--;
    E.dirty = 1;
    ...
    if (E.cy > 0) {
      ...
      prev_line->text =
          realloc(prev_line->text, prev_line->len + line->len + 1);
      if (prev_line->text == NULL) {
        editor_handle_error(ERR_OUT_OF_MEMORY,
                            "Out of memory (merge line realloc).");
        return;
      }
...
void editor_find_next(int direction) {
  ...
  while (1) {
    ...
    if (match) {
      ...
      editor_set_status_message("Found '%s' at %d:%d", E.search_query, E.cy + 1,
                                E.cx + 1);
      editor_refresh_screen();
      return;
    }
...
void paste_from_clipboard() {
  ...
  if (pipe(pipefd) == -1) {
    editor_handle_error(ERR_CLIPBOARD_TOOL,
                        "Paste error: Failed to create pipe.");
    return;
  }
  ...
  if (pid == -1) {
    editor_handle_error(ERR_CLIPBOARD_TOOL,
                        "Paste error: Failed to fork process.");
    close(pipefd[0]);
    close(pipefd[1]);
    return;
  }
  ...
  if (pid == 0) {
    ...
    if (WIFEXITED(status) && WEXITSTATUS(status) == 0) {
      editor_set_status_message("Pasted from clipboard using %s.",
                                clipboard_tool);
    } else {
