...
void editor_read_file(const char *filename) {
    ...
    if (errno == ENOENT) {
      ...
      if (new_line.text == NULL) {
        editor_handle_error(ERR_OUT_OF_MEMORY,
                            "Out of memory (initial line text).");
      }
      editor_lines_array_append(&E->lines, new_line);
      editor_set_status_message("New file: %s", filename);
    } else {
      editor_handle_error(ERR_FILE_OPERATION, "Error opening file '%s': %s",
  ...
  while ((linelen = getline(&line_buffer, &linecap, fp)) != -1) {
    ...
    EditorLine new_line = {.text = malloc(linelen + 1),
                           .len = linelen,
                           .hl = NULL,
                           .hl_open_comment = 0};
    if (new_line.text == NULL) {
  ...
  E->dirty = 0;
  editor_set_status_message("Opened file: %s (%d lines)", filename,
                            E->lines.size);
}
