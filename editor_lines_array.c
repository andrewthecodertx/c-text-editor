...
void init_editor_lines_array(EditorLinesArray *array) {
  array->elements =
      malloc(sizeof(EditorLine) * EDITOR_LINES_ARRAY_INIT_CAPACITY);
...
void editor_lines_array_grow(EditorLinesArray *array) {
  ...
  EditorLine *new_elements =
      realloc(array->elements, sizeof(EditorLine) * new_capacity);
  if (new_elements == NULL) {
    editor_handle_error(ERR_OUT_OF_MEMORY,
                        "Failed to grow EditorLinesArray capacity.");
  }
...
void editor_lines_array_insert(EditorLinesArray *array, int index,
  ...
  if (index < 0 || index > array->size) {
    editor_handle_error(ERR_NONE,
                        "Invalid index for EditorLinesArray insertion.");
    return;
  }
  if (array->size == array->capacity) {
    editor_lines_array_grow(array);
...
void editor_lines_array_delete(EditorLinesArray *array, int index) {
  if (index < 0 || index >= array->size) {
    editor_handle_error(ERR_NONE,
                        "Invalid index for EditorLinesArray deletion.");
    return;
  }
  ...
  if (array->capacity > EDITOR_LINES_ARRAY_INIT_CAPACITY &&
    ...
    if (new_elements == NULL) {
      editor_handle_error(ERR_OUT_OF_MEMORY,
                          "Failed to shrink EditorLinesArray capacity.");
    }
    array->elements = new_elements;
    array->capacity = new_capacity;
  }
